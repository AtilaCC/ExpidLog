...
/* ════════════════════════════════════════════════════════════
   CSS ENTERPRISE — Central de Inteligência Operacional
   Injetado dinamicamente. Compatível com Dark/Light mode.
════════════════════════════════════════════════════════════ */
...
/* ══ Tab base ══ */
#tab-analytics { background: var(--bg); padding-bottom: 84px; }

/* ══ Command Header ══ */
.an-cmd-header {
  display: flex; align-items: center; justify-content: space-between;
  padding: 12px 16px 10px; gap: 10px;
  background: linear-gradient(180deg, rgba(245,158,11,.07) 0%, transparent 100%);
  border-bottom: 1px solid rgba(245,158,11,.14);
}
.an-cmd-brand { display: flex; align-items: center; gap: 10px; }
.an-cmd-icon {
  width: 38px; height: 38px; border-radius: 10px; font-size: 20px;
  display: flex; align-items: center; justify-content: center;
  background: linear-gradient(135deg, rgba(245,158,11,.18) 0%, rgba(245,158,11,.04) 100%);
  border: 1px solid rgba(245,158,11,.28);
  box-shadow: 0 0 14px rgba(245,158,11,.18);
}
.an-cmd-title {
  font-family: 'Barlow Condensed', sans-serif;
  font-size: 15px; font-weight: 900; letter-spacing: 1.5px; color: var(--txt);
}
.an-cmd-sub { font-size: 10px; color: var(--mut); letter-spacing: .2px; margin-top: 1px; }
.an-cmd-controls { display: flex; align-items: center; gap: 8px; }
.an-ts { font-size: 10px; color: var(--mut); white-space: nowrap; }
.an-select {
...
.an-bloco-header--inner { padding: 10px 0 6px; }
.an-live-tag {
  margin-left: auto; font-size: 10px; color: #10b981; font-weight: 700; letter-spacing: .5px;
...
.an-dot-acc    { background: var(--acc); box-shadow: 0 0 6px var(--acc); }
.an-dot-green  { background: #10b981; box-shadow: 0 0 6px #10b981; }
.an-dot-blue   { background: #3b82f6; box-shadow: 0 0 6px #3b82f6; }
.an-dot-purple { background: #8b5cf6; box-shadow: 0 0 6px #8b5cf6; }
.an-dot-red    { background: #ef4444; box-shadow: 0 0 6px #ef4444; }
.an-dot-cyan   { background: #06b6d4; box-shadow: 0 0 6px #06b6d4; }
.an-blink      { animation: an-blink 2s infinite; }
...
@media (max-width: 500px) { .an-kpi-grid { grid-template-columns: repeat(2, 1fr); } }
...
.an-kpi-card:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(0,0,0,.35); }
...
.an-kpi-icon { font-size: 16px; line-height: 1; }
.an-kpi-trend { font-size: 13px; font-weight: 900; font-family: 'Barlow Condensed', sans-serif; }
.an-kpi-val {
  font-family: 'Barlow Condensed', sans-serif; font-size: 24px; font-weight: 900;
  line-height: 1; letter-spacing: -.5px;
}
.an-kpi-label { font-size: 9px; font-weight: 800; color: var(--mut); text-transform: uppercase; letter-spacing: .6px; margin-top: 3px; }
.an-kpi-sub   { font-size: 9px; color: var(--mut); margin-top: 1px; }
.an-kpi-glow  {
  position: absolute; bottom: -8px; right: -8px; width: 48px; height: 48px;
  border-radius: 50%; opacity: .12; filter: blur(12px);
}

/* ══ Exec Card ══ */
.an-exec-card {
  margin: 0 16px 4px; background: var(--bg2); border: 1px solid var(--brd);
...
.an-exec-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }
.an-exec-title  { font-family: 'Barlow Condensed', sans-serif; font-size: 15px; font-weight: 800; letter-spacing: .5px; color: var(--txt); }
.an-exec-status {
...
.an-exec-status.ok   { background: rgba(16,185,129,.15); color: #10b981; border: 1px solid rgba(16,185,129,.25); }
.an-exec-status.warn { background: rgba(245,158,11,.15);  color: var(--acc); border: 1px solid rgba(245,158,11,.25); }
.an-exec-status.crit { background: rgba(239,68,68,.15);   color: #ef4444;  border: 1px solid rgba(239,68,68,.25); }
.an-exec-grid {
  display: grid; grid-template-columns: repeat(4,1fr); gap: 8px; margin-bottom: 12px;
}
.an-exec-item { text-align: center; }
.an-exec-num  { font-family: 'Barlow Condensed', sans-serif; font-size: 26px; font-weight: 900; color: var(--acc); line-height: 1; }
.an-exec-lbl  { font-size: 9px; color: var(--mut); text-transform: uppercase; letter-spacing: .5px; margin-top: 2px; }
.an-exec-resumo { font-size: 11px; color: var(--mut); line-height: 1.6; border-top: 1px solid var(--brd); padding-top: 10px; }

/* ══ Insights ══ */
.an-insights-wrap { padding: 0 16px 4px; display: flex; flex-direction: column; gap: 7px; }
.an-insight {
  display: flex; align-items: flex-start; gap: 10px; padding: 11px 12px;
  border-radius: 10px; border: 1px solid transparent; position: relative; overflow: hidden;
}
.an-insight::before { content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 3px; border-radius: 3px 0 0 3px; }
.an-insight.good { background: rgba(16,185,129,.07);  border-color: rgba(16,185,129,.2); }
.an-insight.good::before { background: #10b981; }
.an-insight.warn { background: rgba(245,158,11,.07);  border-color: rgba(245,158,11,.2); }
.an-insight.warn::before { background: var(--acc); }
.an-insight.crit { background: rgba(239,68,68,.07);   border-color: rgba(239,68,68,.2); }
.an-insight.crit::before { background: #ef4444; }
.an-insight.info { background: rgba(59,130,246,.07);  border-color: rgba(59,130,246,.2); }
.an-insight.info::before { background: #3b82f6; }
.an-insight-icon { font-size: 20px; flex-shrink: 0; }
.an-insight-txt  { font-size: 12px; font-weight: 700; color: var(--txt); }
.an-insight-sub  { font-size: 11px; color: var(--mut); margin-top: 3px; line-height: 1.4; }

/* ══ Charts ══ */
.an-charts-duo {
  display: grid; grid-template-columns: 1fr 1fr; gap: 8px; padding: 0 16px 4px;
}
@media (max-width: 540px) { .an-charts-duo { grid-template-columns: 1fr; } }
.an-chart-block { display: flex; flex-direction: column; }
.an-chart-card {
  background: var(--bg2); border: 1px solid var(--brd); border-radius: 12px;
  padding: 10px; overflow: hidden; margin: 0 16px 4px;
}
.an-chart-card canvas { width: 100% !important; display: block; }
.an-charts-duo .an-chart-card { margin: 0; }
.an-chart-card--heatmap { padding: 10px 10px 6px; }
.an-heatmap-legend {
  display: flex; gap: 8px; justify-content: center; margin-top: 6px;
}
.an-heatmap-legend span {
  font-size: 9px; font-weight: 700; letter-spacing: .5px;
  padding: 2px 8px; border-radius: 4px; color: #fff;
  font-family: 'Barlow Condensed', sans-serif;
}
.an-chart-card--radar { display: flex; align-items: center; justify-content: center; padding: 6px; }
.an-chart-card--radar canvas { width: 100% !important; max-width: 200px; }

/* ══ Rankings ══ */
.an-rankings-grid {
  display: grid; grid-template-columns: 1fr 1fr; gap: 8px; padding: 0 16px 4px;
}
.an-rank-wrap { display: flex; flex-direction: column; gap: 2px; }
.an-rank-item { display: flex; align-items: center; gap: 8px; padding: 8px 0; border-bottom: 1px solid var(--brd); }
.an-rank-pos  { font-size: 16px; width: 24px; text-align: center; flex-shrink: 0; }
.an-rank-info { flex: 1; min-width: 0; }
.an-rank-name { font-weight: 700; font-size: 12px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.an-rank-bar-wrap { height: 3px; background: var(--brd); border-radius: 2px; margin-top: 4px; overflow: hidden; }
.an-rank-bar  { height: 100%; border-radius: 2px; transition: width .7s; }
.an-rank-val  { font-size: 13px; font-weight: 900; font-family: 'Barlow Condensed', sans-serif; flex-shrink: 0; }

/* ══ Transportadora ══ */
.an-transp-card { display: flex; gap: 14px; align-items: center; flex-wrap: wrap; }
.an-transp-canvas { flex-shrink: 0; max-width: 130px; }
.an-transp-legend { flex: 1; min-width: 110px; display: flex; flex-direction: column; gap: 4px; }

/* ══ Equipes ══ */
.an-eq-card {
  display: flex; align-items: center; gap: 12px; padding: 12px 16px;
  background: var(--bg2); border: 1px solid var(--brd); border-radius: 12px;
  margin: 0 16px 7px; transition: border-color .2s, background .2s;
}
.an-eq-card.destaque {
...
.an-eq-pos     { font-size: 22px; width: 30px; text-align: center; flex-shrink: 0; }
.an-eq-name    { font-weight: 700; font-size: 14px; color: var(--txt); }
...
.an-eq-bar     { height: 100%; border-radius: 2px; transition: width .9s; }
.an-eq-metrics { text-align: center; flex-shrink: 0; min-width: 44px; }
.an-eq-val     { font-family: 'Barlow Condensed', sans-serif; font-size: 24px; font-weight: 900; color: var(--acc); }
.an-eq-lbl     { font-size: 10px; color: var(--mut); }

/* ══ Duo (radar + comparativo) ══ */
.an-duo-wrap {
  display: grid; grid-template-columns: 1fr 1fr; gap: 8px; padding: 0 16px 4px;
}
@media (max-width: 540px) { .an-duo-wrap { grid-template-columns: 1fr; } }
.an-duo-block { display: flex; flex-direction: column; }
.an-duo-block .an-chart-card { margin: 0; flex: 1; }

/* ══ Comparativo ══ */
.an-comp-grid { display: grid; grid-template-columns: 1fr auto 1fr; gap: 10px; align-items: center; padding: 4px 0; }
.an-comp-col  { text-align: center; }
.an-comp-header { font-size: 10px; font-weight: 800; color: var(--mut); text-transform: uppercase; letter-spacing: .5px; margin-bottom: 8px; }
.an-comp-val  { font-family: 'Barlow Condensed', sans-serif; font-size: 30px; font-weight: 900; line-height: 1; }
.an-comp-lbl  { font-size: 10px; color: var(--mut); }
.an-comp-val2 { font-size: 11px; color: var(--mut); margin-top: 4px; }
.an-comp-divider { display: flex; flex-direction: column; align-items: center; gap: 5px; }
.an-comp-delta { font-family: 'Barlow Condensed', sans-serif; font-size: 13px; font-weight: 800; }
...
.an-prev-icon  { font-size: 24px; flex-shrink: 0; }
.an-prev-body  { flex: 1; min-width: 0; }
.an-prev-titulo { font-weight: 700; font-size: 13px; }
.an-prev-desc   { font-size: 11px; color: var(--mut); margin-top: 3px; line-height: 1.4; }
.an-prev-val    { font-family: 'Barlow Condensed', sans-serif; font-size: 18px; font-weight: 900; flex-shrink: 0; text-align: right; }

/* ══ Gargalos ══ */
.an-garg-card {
  background: var(--bg2); border: 1px solid rgba(239,68,68,.22);
  border-left: 3px solid #ef4444; border-radius: 0 10px 10px 0;
  padding: 12px 14px; margin: 0 16px 7px;
}
.an-garg-titulo { font-weight: 800; font-size: 13px; color: #ef4444; margin-bottom: 4px; }
.an-garg-desc   { font-size: 12px; color: var(--txt); margin-bottom: 6px; line-height: 1.5; }
.an-garg-sug    { font-size: 11px; color: var(--mut); }

/* ══ Empty ══ */
.an-empty { display: flex; flex-direction: column; align-items: center; gap: 8px; padding: 24px 16px; color: var(--mut); font-size: 12px; text-align: center; }
.an-empty-icon { font-size: 28px; opacity: .5; }

/* ══ Misc ══ */
.an-pb-nav  { padding-bottom: 8px; }

/* ══ Light mode overrides ══ */
body.light .an-exec-card::after { background: linear-gradient(90deg, rgba(245,158,11,.6), transparent); }
body.light .an-insight.good { background: rgba(16,185,129,.06); }
body.light .an-insight.warn { background: rgba(245,158,11,.06); }
body.light .an-insight.crit { background: rgba(239,68,68,.06); }
body.light .an-insight.info { background: rgba(59,130,246,.06); }
