...
CREATE TABLE IF NOT EXISTS push_subscriptions (
  ...
  user_agent TEXT, ativo BOOLEAN NOT NULL DEFAULT true,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS automacao_regras (
  ...
  condicao JSONB NOT NULL DEFAULT '{}', acao TEXT NOT NULL,
  params JSONB NOT NULL DEFAULT '{}', ativo BOOLEAN NOT NULL DEFAULT true,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS automacao_log (
  ...
  regra_nome TEXT, acao TEXT, params JSONB, resultado TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS idx_ia_insights_empresa  ON ia_insights(empresa_id);
CREATE INDEX IF NOT EXISTS idx_ia_insights_created  ON ia_insights(created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_ia_aprendizado_empresa ON ia_aprendizado(empresa_id);
`;

async function migrate() {
  logger.info('🔄 Starting migration v5 (Fase 15)...');
  ...
  try {
    ...
    logger.info('  → Applying Fase 15 schema (IA Autônoma)...');
    await client.query(SCHEMA_FASE15);
