...
function gerarMsg() {
  ...
  const vals = {
    ...
    // Carga — só aparece na mensagem se avaliada
    carga_estado:          estadoInfo ? estadoInfo.label : '',
    carga_estado_emoji:    estadoInfo ? estadoInfo.emoji : '',
    carga_problemas_linha: problemas ? `• Problemas: ${problemas}` : '',
  };
  ...
  if (_tmplMode === 'siren') {
    ...
    // Adiciona bloco de carga SOMENTE se o operador avaliou
    if (estado && estadoInfo) {
  ...
  // Remove linhas vazias excessivas e placeholders não substituídos
  msg = msg.replace(/\[carga_[^\]]+\]/g, '').replace(/\n{3,}/g, '\n\n').trim();
